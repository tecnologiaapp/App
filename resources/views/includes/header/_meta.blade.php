<meta charset="utf-8">
<title>Agencia APP</title>
<meta name="description" content="Agencia APP - Alcaldía de Medellín">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
<!--Favicon-->
<link rel="icon" type="image/png" href="{{ asset('assets/img/logo-white.png')}}">
<!--Framework-->
<link rel="stylesheet" href="{{ asset('assets/css/framework/bootstrap.min.css')}}">
<!--Plugins-->
<link rel="stylesheet" href="{{ asset('assets/css/vlt-plugins.css')}}">
<!--Fonts-->
<link href="https://fonts.googleapis.com/css?family=Montserrat:400,500,600,700,800%7CMuli:400,400i,600%7CPlayfair+Display:700i" rel="stylesheet">
<link rel="stylesheet" href="{{ asset('assets/fonts/leedo/style.css')}}">
<link rel="stylesheet" href="{{ asset('assets/fonts/roger/roger.css')}}">
<link rel="stylesheet" href="{{ asset('assets/fonts/boucher/boucher.css')}}">
<link rel="stylesheet" href="{{ asset('assets/fonts/oldventure/oldventure.css')}}">
<link rel="stylesheet" href="{{ asset('assets/fonts/elephant/elephant.css')}}">
<meta name="facebook-domain-verification" content="09e9i3q69xdt1effv1d2ao77zx2lgy" />
<!--Icons-->
<link rel="stylesheet" href="{{ asset('assets/fonts/fontawesome/font-awesome.min.css')}}">

<!--Style-->
<link rel="stylesheet" href="{{ asset('assets/css/vlt-stylos.css')}}">
<!--Custom-->
<link rel="stylesheet" href="{{ asset('assets/css/customi.css')}}">